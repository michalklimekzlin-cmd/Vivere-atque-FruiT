...
(() => {
  ...
  function step(index, amount, button) {
    ...
    if (index === 1) {
      ai.activeGlyph = glyph(index);
      saveAI("změna glyphu A|lı.");
    }
  ...
  function paint(button, index) {
    const values = DRUMS[index].glyphs;
    const at = state.indices[index];

    button.dataset.size =
      Array.from(glyph(index)).length > 3 ? "wide" :
      Array.from(glyph(index)).length > 1 ? "medium" : "normal";
  ...
  function open(index) {
    if (DRUMS[index].action === "debug") {
      if (window.CHT360UI?.openDebug) {
        window.CHT360UI.openDebug();
      } else {
        window.dispatchEvent(new CustomEvent("cht.ui.debug.open"));
      }
      return;
    }
  ...
  function bind(button, index) {
    ...
    button.addEventListener("pointermove", event => {
      ...
      step(index, next - drag.step, button);
      drag.step = next;
      drag.moved = true;
    });
    ...
    button.addEventListener("keydown", event => {
      if (event.key === "ArrowUp" || event.key === "ArrowDown") {
        event.preventDefault();
        step(index, event.key === "ArrowUp" ? 1 : -1, button);
      }
    });
  }
  ...
  function render() {
    ...
    DRUMS.forEach((drum, index) => {
      ...
      button.type = "button";
      button.className = "cht360OsmZamek cht360ExtraDrum";
      button.dataset.extraDrum = String(index);
      button.setAttribute(
        "aria-label",
        drum.label + ". Klepnutím otevřeš funkci, tahem měníš Glyph."
      );

      button.innerHTML =
        '<span class="cht360OsmZamekLabel"></span>' +
        '<span class="cht360OsmZamekGhost" data-reel="prev"></span>' +
        '<strong class="cht360OsmZamekGlyph" data-reel="now"></strong>' +
        '<span class="cht360OsmZamekGhost" data-reel="next"></span>';

      button.querySelector(".cht360OsmZamekLabel").textContent = drum.label;
  ...
  function openAI() {
    ...
    if (!panel) {
      ...
      panel.id = PANEL;
      panel.className = "cht360AiBrana";
      panel.hidden = true;

      panel.setAttribute("role", "dialog");
      panel.setAttribute("aria-modal", "true");
      panel.setAttribute("aria-label", "A|lı. — brána AI");

      panel.innerHTML =
        '<div class="cht360AiBranaCard">' +
          '<header>' +
            '<div><strong>A|lı.</strong><span>místní brána pro nástroje AI</span></div>' +
            '<button type="button" data-ai="close" aria-label="Zavřít">×</button>' +
          '</header>' +
          '<p class="glyph" data-ai-glyph></p>' +
          '<p class="state" data-ai-state></p>' +
          '<div class="actions">' +
            '<button type="button" data-ai="revia">Otevřít Revii</button>' +
            '<button type="button" data-ai="mluva">Otevřít Mluvu</button>' +
            '<button type="button" data-ai="connect">Připojit AI</button>' +
            '<button type="button" data-ai="disconnect">Odpojit</button>' +
          '</div>' +
          '<small>Žádný klíč ani obsah se odsud nikam neposílá. Plnohodnotného poskytovatele připojíš až svým nastavením.</small>' +
        '</div>';
      ...
      panel.addEventListener("click", event => {
        ...
        if (action === "revia") {
          closeAI();
          document.getElementById("openReviaPanel")?.click();
        }

        if (action === "mluva") {
          window.location.assign(new URL("mluva-cht-360/", location.href));
        }
  ...
  function paintAI() {
    ...
    panel.querySelector("[data-ai-state]").textContent =
      ai.status === "připojeno"
        ...
        : ai.status === "čeká"
          ? "Čeká na zvolenou AI"
          : "Odpojeno · Revia a Mluva zůstávají dostupné";
  }

  function connectAI() {
    const detail = {
      glyph: glyph(1),
      source: "cht360-devaty-desaty-bubinek",
      requestedAt: new Date().toISOString()
    };

    ai.status = "čeká";
    saveAI("žádost o připojení AI");

    window.dispatchEvent(new CustomEvent("cht.ai.connect.request", { detail }));

    if (typeof window.CHT360AI?.connect === "function") {
      Promise.resolve(window.CHT360AI.connect(detail))
        .then(result => {
          ai.status = "připojeno";
          ai.provider = String(result?.provider || result?.name || "A|lı.");
          saveAI("připojení AI");
          paintAI();
        })
        .catch(() => {
  ...
  function disconnectAI() {
    ...
    if (typeof window.CHT360AI?.disconnect === "function") {
      Promise.resolve(window.CHT360AI.disconnect(detail)).catch(() => {});
    }
