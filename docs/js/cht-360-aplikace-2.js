...
function drawDigitalEarthLand(x, y, radius, spin) {
  context.save();
  context.translate(x + Math.sin(spin) * radius * .055, y);
  ...
  /*
    Stylizovaná silueta České republiky: široká česká kotlina,
    západní Čechy vlevo a moravský výběžek vpravo.
  */
  context.beginPath();
  ...
  /* Tři tiché světelné body drží uvnitř mapy digitální strukturu. */
  [
    [-.40, -.04],
    [.03, -.10],
    [.42, .04]
    ...
    context.beginPath();
    context.arc(px * radius, py * radius, Math.max(1.05, radius * .035), 0, Math.PI * 2);
...
function drawDigitalEarthGlyphs(position, radius, time) {
  ...
  EARTH_ORBIT_GLYPHS.forEach((glyph, index) => {
    ...
    const glow = context.createRadialGradient(x, y, 1, x, y, nodeRadius * 1.8);
    ...
    context.font = "800 " + Math.max(6, Math.round(nodeRadius * .72)) + "px system-ui";
    context.textAlign = "center";
    context.textBaseline = "middle";
...
function drawDigitalEarthCore(core, time) {
  ...
  halo.addColorStop(.44, active ? "rgba(255,167,43,.38)" : "rgba(255,167,43,.22)");
  ...
  for (let ring = 0; ring < 3; ring += 1) {
    ...
    context.strokeStyle = "rgba(255,190,68," + (.35 - ring * .09) + ")";
    context.lineWidth = ring === 0 ? 1.2 : .7;
    context.stroke();
  }
  ...
  drawDigitalEarthLand(position.x, position.y, radius, spin);
  context.restore();

  /* Žhavý okraj atmosféry. */
  context.shadowColor = active ? "rgba(255,211,116,.95)" : "rgba(255,165,45,.82)";
  context.shadowBlur = active ? radius * .34 : radius * .22;
  context.strokeStyle = active ? "rgba(255,238,184,.96)" : "rgba(255,187,69,.88)";
  ...
  /* Dva lehce nakloněné digitální prstence. */
  context.strokeStyle = "rgba(255,211,119,.30)";
  context.lineWidth = .8;
  context.beginPath();
  context.ellipse(position.x, position.y, radius * 1.20, radius * .29, -.18, 0, Math.PI * 2);
  context.stroke();
  context.beginPath();
  context.ellipse(position.x, position.y, radius * 1.28, radius * .34, .26, 0, Math.PI * 2);
  context.stroke();
  ...
  context.font = "900 " + Math.max(7, Math.round(radius * .16)) + "px system-ui";
  context.fillText("ZEMĚ", position.x, position.y + radius * .47);
  context.fillStyle = "rgba(255,226,168,.76)";
  context.font = "700 " + Math.max(6, Math.round(radius * .12)) + "px system-ui";
  context.fillText(stats.used + "/70", position.x, position.y + radius * .67);

  /* Glyphy se objeví až po klepnutí na Zemi a otevření její Paměti. */
  if (active) {
    drawDigitalEarthGlyphs(position, radius, time);
  }

  context.restore();

  /* Zachování původní klikací plochy a všech funkcí jádra earth. */
  core.position = position;
...
/*
  Vnější vrstvy stylů mohou kreslit vlastní vzhled konkrétní koule,
  ale motor CHT dál vlastní polohu, dotyk, Paměť i pořadí v 3D scéně.
  Když se stylový soubor nenačte, použije se bezpečně zdejší záloha.
*/
function drawExternalCoreStyle(core, time) {
  const renderer = window.CHT360CoreStyles?.[core.id];
  ...
  const scale =
    ...
    position.perspective;
  const radius = core.radius * scale;
  const active = Boolean(selectedCore && selectedCore.id === core.id);
  ...
  try {
    ...
    core.position = result?.position || position;
    core.drawRadius = Number.isFinite(result?.drawRadius)
      ? result.drawRadius
      : radius;
    ...
    console.warn(
      "[CHT 360°‰.] Styl koule se nepodařilo načíst; použita záloha.",
      error
    );
    return false;
  }
...
function drawCore(core, time) {
  ...
  if (drawExternalCoreStyle(core, time)) {
    return;
  }
