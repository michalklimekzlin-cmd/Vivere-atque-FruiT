...
import {
  getLoadedVercelRepositoryMemory,
  getVercelRepositoryMemoryStatus,
  loadVercelRepositoryMemory,
  searchVercelRepository
} from "./revia-vercel-memory.js";
...
function loadState() {
  ...
  return {
    ...
    messages: [{
      role: "revia",
      text: "Jsem Revia v oběhu CHT 360°‰. Místní mapu mám hned; úplnou paměť repozitáře drží Vercel a připojím ji jen na tvůj pokyn. Pro zdroj napiš třeba „repo: název funkce“ nebo klepni na Repo."
    }]
  };
}
...
function formatRemoteMemoryReady(memory) {
  const snapshot = memory?.snapshot || {};
  return [
    "Vercelová paměť repozitáře je připojená.",
    `• ${Number(snapshot.trackedFileCount) || 0} souborů · ${Number(snapshot.sourceLines) || 0} řádků · ${Number(snapshot.functionCount) || 0} rozpoznaných částí.`,
    "• Napiš „repo: název funkce nebo souboru“ a Revia vrátí konkrétní cestu i odpovídající řádky.",
    "• Zdroj jen čtu; žádný nalezený soubor tím nespustím, nezměním ani neodešlu."
  ].join("\n");
}

async function openRemoteRepositoryMemory({ force = false } = {}) {
  if (!navigator.onLine) {
    const known = getVercelRepositoryMemoryStatus();
    const suffix = known.loadedAt ? " Poslední úspěšné připojení: " + new Date(known.loadedAt).toLocaleString("cs-CZ") + "." : "";
    push("revia", "Úplná paměť repozitáře je na Vercelu, takže bez připojení ji teď nemohu načíst." + suffix);
    setState("Revia zůstává lokální; Vercelová paměť čeká na připojení.");
    return null;
  }

  setState("Načítám úplnou paměť repozitáře z Vercelu na tvůj pokyn…");
  try {
    const module = await loadVercelRepositoryMemory({ force });
    rememberActivity("repository", "Revia připojila úplnou paměť repozitáře z Vercelu.");
    push("revia", formatRemoteMemoryReady(module.REVIA_REPOSITORY_MEMORY));
    setState("Úplná paměť repozitáře je připojená z Vercelu.");
    return module;
  } catch (error) {
    const message = "Paměť z Vercelu se teď nepodařilo připojit. Místní CHT běží dál beze změny.";
    rememberActivity("repository", message);
    push("revia", message);
    setState(message);
    console.warn("[Revia] Vercelová paměť se nenačetla.", error);
    return null;
  }
}

async function requestRepositorySearch(query) {
  if (!navigator.onLine) {
    push("revia", "Hledání v celém repozitáři je uložené na Vercelu. Připoj se a zkus znovu „repo: " + query + "“.");
    return;
  }

  setState("Hledám v úplné paměti repozitáře na Vercelu…");
  try {
    const result = await searchVercelRepository(query, 8);
    rememberActivity("repository", "Revia dohledala ve Vercelové paměti zdroje pro „" + String(query).slice(0, 120) + "“.");
    push("revia", result.text);
    setState("Hledání ve Vercelové paměti je hotové.");
  } catch (error) {
    const message = "Hledání ve Vercelové paměti se teď nepodařilo dokončit. Místní Paměť CHT zůstala beze změny.";
    rememberActivity("repository", message);
    push("revia", message);
    setState(message);
    console.warn("[Revia] Hledání ve Vercelové paměti selhalo.", error);
  }
}
...
function replyFor(message) {
  ...
  if (repositorySearchMatch) {
    ...
    void requestRepositorySearch(query);
    return "Na tvůj pokyn otevírám úplnou paměť repozitáře z Vercelu a hledám „" + query.slice(0, 140) + "“…";
  }
  ...
  if (/(funkce|soubor|radek|řádek|zdroj|kod|kód)/.test(text)) {
    return "Pro hledání ve všech souborech napiš „repo: název funkce nebo souboru“. Paměť se pak výslovně načte z Vercelu.";
  }
...
function runAction(action) {
  ...
  if (action === "repository") {
    void openRemoteRepositoryMemory();
    return;
  }
  if (action === "plans") {
  ...
  if (action === "mluva") {
    rememberActivity("navigation", "Přechod do samostatné Mluvy CHT.");
    window.location.assign("./mluva-cht-360/");
    return;
  }
  if (action === "glyph") {
...
window.CHT360Revia = Object.freeze({
  ...
  getRepositoryMemory: () => getLoadedVercelRepositoryMemory(),
  getRepositoryMemoryStatus: () => getVercelRepositoryMemoryStatus(),
  loadRepositoryMemory: options => openRemoteRepositoryMemory(options),
  refreshRepositoryMemory: () => openRemoteRepositoryMemory({ force: true }),
  ...
  searchRepository: async text => (await searchVercelRepository(text)).results,
